...
from supar.models.sdp import TriaffineSemanticDependencyModel
...
class BiaffineSemanticDependencyParser(Parser):
    ...
    def _train(self, loader):
        ...
        for i, batch in enumerate(bar, 1):
            words, *feats, labels, init_labels = batch
            # one = torch.ones_like(labels, dtype=torch.float32)
            # zero = torch.full_like(labels, 0.01, dtype=torch.float32)
            # adj = torch.where(labels != -1, labels.float(), zero)

            adj = init_labels.float()
            adj = F.softmax(adj, dim=1)

            # adj = torch.ones_like(init_labels, dtype=torch.float32, requires_grad=True)
            # adj = F.softmax(adj, dim=1)
    ...
    def _evaluate(self, loader):
        ...
        for batch in loader:
            words, *feats, labels, init_labels = batch
            # zero = torch.full_like(labels, 0.01, dtype=torch.float32)
            # adj = torch.where(labels > 0, labels.float(), zero)
            adj = init_labels.float()
            adj = F.softmax(adj, dim=1)
            word_mask = words.ne(self.args.pad_index)
            mask = word_mask if len(words.shape) < 3 else word_mask.any(-1)
    ...
    def build(cls, path, min_freq=7, fix_len=20, **kwargs):
        ...
        # TODO
        INIT_LABEL = ChartField('init_labels', fn=CoNLL.get_labels)
        transform = CoNLL(FORM=(WORD, CHAR, ELMO, BERT), LEMMA=LEMMA, POS=TAG, PHEAD=LABEL, PDEPREL=INIT_LABEL)
...
class TriaffineSemanticDependencyParser(Parser):
    r"""
    The implementation of Biaffine Semantic Dependency Parser :cite:`dozat-etal-2018-simpler`.
    """

    NAME = 'triaffine-semantic-dependency'
    MODEL = TriaffineSemanticDependencyModel
