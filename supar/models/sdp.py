...
from supar.modules.gnn import GCN
...
class BiaffineSemanticDependencyModel(Model):
    ...
    def forward(self, words, feats=None, adj=None):
        r"""
        Args:
...
class TriaffineSemanticDependencyModel(Model):
    r"""
    The implementation of Biaffine Semantic Dependency Parser :cite:`dozat-etal-2018-simpler`.
    ...
    def __init__(self,
        ...
        self.edge_attn = Triaffine(n_in=n_edge_mlp, n_out=2, bias_x=True, bias_y=True)
        self.label_attn = Triaffine(n_in=n_label_mlp, n_out=n_labels, bias_x=True, bias_y=True)
        self.criterion = nn.CrossEntropyLoss()
